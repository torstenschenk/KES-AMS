...
int main(int argc, char **argv)
{
    ...
    double senkrechte = V01(1)*V12(2) - (V12(1)*V01(2));
    double dp =DotProduct(V01,V12);
    //double beta = (acos(dp/L01/L12)/2)  * (senkrechte/abs(senkrechte));
    //beta = 180 - beta;
    thetaL = ( (acos(dp/(L01*L12))/2) ) * (senkrechte/fabs(senkrechte));
    double fthetaL =fabs(thetaL);
    // Parameter für krümmungsstetigen Übergang mit Klothoide berechnen
    L = fthetaL * 2 * Rmin;
    xL = L * ( 1- (fthetaL*fthetaL/10) + (pow(fthetaL,4)/216) );
    yL = L * ( fthetaL/3 - pow(fthetaL,3)/42 + pow(fthetaL,5)/1320);
    delta = yL*tan(fthetaL);//xL + yL*tan(thetaL);

    // Berechnung des Start- und des Endpunktes der Klothoiden
    Pc0 = (V01/L01)*(L01-delta-xL) + P0;
    Pc1 = (V12/L12)*(delta+xL) + P1;
	...
	robot.log.info("phi: %.2lf m.", acos(dp/(L01*L12))*180/PI);
	robot.log.info("thetaL: %.2lf m.", thetaL*180/PI);
    ...
    DriveRobot(&robot, L1, L, L2,(senkrechte>=0)? 1 : -1);
