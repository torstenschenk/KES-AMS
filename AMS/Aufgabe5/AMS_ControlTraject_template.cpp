...
int main(int argc, char **argv)
{
    ...
    theta = std::atan2( (y_s1-y_s) , (x_s1-x_s) );
    ...
    while( infile >> x_s >> y_s ) { // Schleife über alle Punkte der Trajektorie
        ...
        x +=  delta * std::cos(theta + phi/2.0);
        y +=  delta * std::sin(theta + phi/2.0);
        theta += phi;
        ...
        a1 = ax * std::cos(theta) + ay * std::sin(theta);
        // is ignored
        a2 = (-ax * std::sin(theta)) + ay * std::cos(theta);
        v_s += a1 * T;
        w_s = a2 / v_s;
