...
bool KalmanFilter::Correction(double& x, double& y, double& theta)
{
    ...
    int i =0;
       Phi = rtod(theta + PhiR);
    d = dR + x*cos(dtor(Phi)) + y*sin(dtor(Phi));
 cout << i++ << endl; //0
    if(d < 0){
        d = fabs(d);
        Phi += 180;
...
cout << i++ << endl;//3
    ...
    if(!map.search(Phi,d,1,S)){
        cout << "No Wall detection...." << endl;
        return false;
    }
    cout << i++ << endl;//4
    ...
    if(normalize(dtor(y_(2)-theta)) > 0.5)
    {
        y_(1) = (d+dR);
        y_(2) = normalize(dtor(Phi)-PhiR+M_PI);
    }
cout << i++ << endl;//5
